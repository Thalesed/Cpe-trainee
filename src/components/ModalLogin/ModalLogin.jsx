...
const ModalLogin = () => {
  ...
  // UseEffect substitui a função carregarNomesProjetos
  useEffect(() => {
    if (projetos) {
      const nomes = projetos.map((projeto) => projeto?.nome);
  ...
  return (
    <Grid className="ModalLogin">
      <Titulo>
        <text>Confirmação de Login</text>
      </Titulo>

      <Input
        text="* Como deseja logar?"
        placeH={"Presencial/Remoto"}
        opcoes={["Presencial", "Remoto"]}
      ></Input>

      <Input
        text="* O que você pretende fazer nesse horário?"
        placeH="Selecione a tarefa"
        opcoes={[
          "Outros",
          "Tarefas/Operacional",
          "Equipes Paralelas",
      ...
      <InputBox
        ...
        placeH="Descrição da atividade exercida"
      />
